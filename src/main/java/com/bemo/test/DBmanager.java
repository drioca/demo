   ...
   public void insert () {
      ...
      String iNs = "INSERT INTO test (age) VALUES (" + Integer.toString(UserData.age) +");";
        ...
        try {
         ...
         stmtc.executeUpdate("CREATE TABLE IF NOT EXISTS public.test (key_column bigserial NOT NULL DEFAULT nextval('test_key_column_seq'::regclass),age numeric(131089,0),timestamp timestamptz,ctimestamp timestamptz DEFAULT now(),CONSTRAINT test_pkey PRIMARY KEY (key_column));CREATE UNIQUE INDEX test_pkey ON public.test (key_column);");
